...
export class DatabaseService {
	...
	static async createProfile(profileId: string): Promise<void> {
		const { error } = await supabase.from('profiles').insert({ id: profileId }).select().single();
	...
	static async getTotalUserDebt(userId: string): Promise<number> {
		...
		return (data || []).reduce((sum: any, debt: any) => sum + debt.amount, 0);
	}
	...
	static async getTotalMonthlyPayments(userId: string): Promise<number> {
		try {
			...
			const totalDebtPayments = (debts || []).reduce((sum : any, debt: any) => sum + debt.minimum_payment, 0);

			// Calculate total expenses
			const totalExpenses = (expenses || []).reduce((sum : any, expense : any) => sum + expense.amount, 0);
