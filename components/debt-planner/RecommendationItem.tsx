    ...
    >
      <Text
        style={[
          ...
          { backgroundColor: tintColor, color: isDark ? "#000" : "#fff" },
        ]}
      >
        {index + 1}
      </Text>
      <View style={styles.recommendationDetails}>
        ...
        {!isFinite(payoffTime) ? (
          ...
          <>
            <Text style={[styles.recommendationTime, { color: iconColor }]}>
              Payoff Time: {Math.floor(payoffTime / 12)} years, {Math.round(payoffTime % 12)} months
            </Text>
            <Text style={[styles.recommendationInterest, { color: iconColor }]}>
              Total Interest: ${isFinite(totalInterest) ? totalInterest.toLocaleString() : "--"}
            </Text>
          </>
        )}
      </View>
